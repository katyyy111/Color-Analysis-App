...
interface QuizPhotoFlowProps {
    saveSamples: (questionIndex: number, samples: ColorSampleDataSet[]) => void
    setFrontFacingImage: (data: string | null) => void
    onComplete: () => void
}

export function QuizPhotoFlow({ saveSamples, setFrontFacingImage, onComplete}: QuizPhotoFlowProps) {
  ...
  function handlePickedColors(samples: ColorSampleDataSet[]) {
    saveSamples(current, samples)
    if (!isLastStep) {
      setReadyToCollectColors(false);
      setCurrent((prev) => prev + 1)
      return
    }
    setFrontFacingImage(photos[0])
    onComplete()
  }
